...
public class ArriveeQuery {
    ...
    // V35 : ville directe des sites (sites.ville_id) prioritaire côté arrivée (vsa)
    // et côté départ (vsd), chaîne quartier→commune→ville en fallback (COALESCE).
    public static final String SELECT_BASE = """
        SELECT a.arrivee_id, a.arrivee_uuid, a.site_id, a.depart_id, a.libelle,
               ...
               COALESCE(vsa.ville_uuid, v.ville_uuid) AS ville_uuid,
               COALESCE(vsa.libelle, v.libelle) AS ville_libelle,
               COALESCE(rsa.libelle, r.libelle) AS region_libelle,
               -- Départ
        ...
        LEFT JOIN villes vsa ON s.ville_id = vsa.ville_id
    ...
    public static final String FIND_BY_VILLE_ARRIVEE = SELECT_BASE +
            " WHERE COALESCE(vsa.ville_uuid, v.ville_uuid) = :villeUuid AND a.actif = true ORDER BY a.ordre_affichage ASC, a.libelle ASC";

    public static final String FIND_BY_VILLE_DEPART = SELECT_BASE +
            " WHERE COALESCE(vsd.ville_uuid, vd.ville_uuid) = :villeUuid AND a.actif = true ORDER BY a.ordre_affichage ASC, a.libelle ASC";

    public static final String FIND_BY_DEPART_AND_VILLE_ARRIVEE = SELECT_BASE +
            " WHERE d.depart_uuid = :departUuid AND COALESCE(vsa.ville_uuid, v.ville_uuid) = :villeArriveeUuid AND a.actif = true ORDER BY a.ordre_affichage ASC";
